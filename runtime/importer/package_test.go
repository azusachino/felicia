...
func TestDecodePackageNormalizesRouteMementoAndMedia(t *testing.T) {
	pkg := &journeypackage.Package{
		...
		Files: map[string][]byte{
			...
			// `transit` is edge-anchored (core/kinds/transit.yaml), so its geometry
			// is a line, not a point: a single point here is the corruption issue
			// #77 records, and the import boundary now rejects it.
	...
	if line, ok := got.Mementos[0].Geom.(orb.LineString); !ok || len(line) != 2 {
		t.Fatalf("edge-anchored geometry was not normalized to a line: %#v", got.Mementos[0].Geom)
	}
