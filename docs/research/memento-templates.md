...
> 2026-07-09. felicia needs to author *custom data types from the UI*: fill a form for a
...
> [transit creator](transit-tickets.md) into the mechanism behind *every* authored kind, and
...
genuinely proliferate (the collector niche — *eki* stamps, *goshuin*, character goods,
*omiyage* — is the whole point, `mementos-not-tickets.md`). So the field list is declared
...
- **Form** — the transit creator (`transit-tickets.md`) is now just *the `transit` template
  rendered*. A new kind ships a `.yaml` + a stub component; **no new form code**.
...
Dawarich + Immich (`source-connectors.md`) — lands mementos through the *same* seam: an
...
- **`kind` must be registered.** Validating against an unknown kind is itself an error
  ...
  legal set *is* the registry (D8/D9).
...
- **Stub-template binding.** `stub: transit` is a frontend component id today. Whether the
  ...
  component (liuaaron-style, `liuaaron-teardown.md`) is a *frontend* question — deferred; the
