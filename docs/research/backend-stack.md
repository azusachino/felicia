...
| Migrations         | **goose**                                                            | Already the project choice; pure CLI, numbered SQL files, Go migrations when a data migration needs to query. Atlas (declarative, drift detection, CI diff) is a _future_ option but heavier than a research-stage repo needs; goose stays.                                                                                                                                                                                                       |
...
| EXIF read          | **imaging fork's EXIF, else `dsoprea/go-exif/v3`**                   | We only need to _read_ lat/lng/timestamp. Prefer the metadata read already in the imaging fork to avoid a dep; if insufficient, `dsoprea/go-exif/v3` is actively maintained (read+write, HEIC). `rwcarlsen/goexif` works but is effectively frozen.                                                                                                                                                                                               |
...
demo's pre-formatted `"2026年5月12日"` / `"JPY 210"` are a _view_ concern.
...
1. **Admin authoring form** — the field list + `type`/`widget` generate the creator form
   (the transit creator, `transit-tickets.md`, becomes _one instance_ of this).
...
**Guardrail (anti-DSL).** The template describes _fields, an anchor, and refs_ — nothing
