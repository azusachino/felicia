...
> 2026-06-12. Triggered by a real question: _the CLI import feels unfriendly — what if I
> want to sell this?_ This note records the analysis so the **why** behind the
> [direction](../direction.md) survives. Outcome ADR: `felicia:decision:personal-now-product-ready`.

## The complaint, diagnosed

`waypoints import --immich-album "Jeju"` feels bad as a _product_ surface. But the CLI
...
| --------------------------------------------- | -------------------------------------------------- |
| Self-hosted Dawarich — _you_ run a GPS logger | Customers don't run loggers — **the hard problem** |
...
_their own app_, automatic photo import, a beautiful map, monetized with printed photo
books. Also in the space: Journi, Wanderlog, plus Strava / Google Timeline adjacent.

To sell against that you must out-execute on _something_: design taste (the memento → essay
...
> differentiation is _object-first authored scrapbook_ vs. _log_. Polarsteps, Dawarich, and
...
> **collector mindset** (_eki_ stamps, _goshuin_, character goods, _omiyage_) — a taste-niche
> no log occupies.
...
- **Companion app that logs in the background** — i.e. you are now building Polarsteps' app.
  This _is_ the moat and _is_ the pain.
...
interfaces). That core _is_ the product engine in either future — only the **sources** and
the **trigger** change:
...
So building the personal version _with clean seams_ costs almost nothing extra and keeps
...
- Differentiation thesis vs. Polarsteps — sharpened to _object-first scrapbook vs. log_
