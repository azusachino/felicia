...
> Notion template _before_ a spec freeze, then implement on our own stack. De-risks the open
> question — _does the `Memento` model hold one shape across kinds?_ — where it's cheap to
...
| ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| Schema — does `Memento` + `kind` hold across ticket/goods/receipt/stamp? Relations _are_ the FKs | The **map** — no geo; route / GPX / click-stub-on-map is the index metaphor |
...
| stub_image  | files            | photographed stub _or_ (later) generated card       |
| animation   | select           | open style                                          |
| seq         | number           | order along the route                               |
| _essay_     | **page body**    | where Notion shines — the authored piece            |

**Photos** _(mirrors `MEMENTO_PHOTO`)_
...
`gpx`/`map-link` as out-of-band fields — _are_ the map layer. Their awkwardness in Notion is
