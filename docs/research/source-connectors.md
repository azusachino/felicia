...
gives full control: *all* photos (not just the geolocated subset Dawarich's `/photos` surfaces
...
*Reference — how Dawarich calls Immich:* `GET /api/v1/photos?start_date&end_date` →
...
and per-photo thumbnails (`/api/v1/photos/:id/thumbnail.jpg?source=immich`). We copy the *call
and caching shape*, not the proxy hop — and confirm auth needs Immich key scopes `asset.read` +
`asset.view`.
...
- **Client generation** — Immich publishes OpenAPI, but felicia touches only `POST
  ...
  endpoints) is hand-written in `internal/dawarich` — no codegen. *(resolves the prior two Open
  bullets: Dawarich has no OpenAPI but a small stable REST surface.)*
