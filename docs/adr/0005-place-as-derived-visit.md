...
1. **Consume Dawarich Semantic Data:** Dawarich already implements a robust pipeline (`points -> tracks -> visits @ places -> trips`). When importing data, *felicia* consumes Dawarich's `visits` and `places` endpoints directly.
